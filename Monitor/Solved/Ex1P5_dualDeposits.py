...
class Buffer:
    def __init__(self, nb_cases):
        ...
        self.nb_production = Value('i',2, lock=False)
        self.verrou = Lock()
    ...
    def produce(self, msg_val, msg_type, msg_source):
        with self.verrou:
            while self.nb_empty.value == 0 or self.nb_production.value == 0:
                self.acces[0].wait()
            self.do_production(msg_val, msg_type, msg_source)
            self.nb_production.value -= 1
            if(self.nb_empty.value == 0):
                self.nb_production.value = 2
            self.acces[1].notify()
    ...
    def consume(self, id_cons):
        with self.verrou:
            while self.nb_empty.value == self.nb_cases or self.nb_production.value == 1:
                self.acces[1].wait()
